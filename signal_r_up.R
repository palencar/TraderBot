...
for(dt in seq(as.Date(args[2]), as.Date(args[1]), by = "-1 day"))
{
  ...
  for(symbol in filterSymbols)
  {
    ...
    #if(length(alertas) > 0)
    #{
    #  objectName <- sprintf("backtest/%s-%s_%d_%d_turnpoints.Robj", chartDate, symbol, k1, k2)
    #  
    #  dput(alertas, objectName)
    #}
    #if(length(alertas) == 0)
    ...
    #if(length(alertas) > 0)
    #{
    #  objectName <- sprintf("backtest/%s-%s_%d_%d_turnpoints_r_up.Robj", chartDate, symbol, k1, k2)
    #  
    #  dput(alertas, objectName)
    #}
    #if(length(alertas) == 0)
